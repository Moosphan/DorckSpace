...
## 1.1 当前实现状态（2026-08-28）

第一版账户快照和本地重置观测已经完成，但此前公开信号只接入 OpenAI Status，导致官方在 X 发布的重置公告不会写入时间轴。当前已补齐以下链路：

- 通过公开的 Dayclaw X 源读取 Codex 负责人 `@thsottiaux` 的公开动态；不读取 X 登录态，也不保存 X Cookie。
- 对动态标题和正文做确定性关键词过滤，识别 Codex/ChatGPT Work 用量、限额与 reset 相关公告，并按发布时间倒序。
- 使用 X 帖子 ID 作为唯一键写入 `reset_radar_history`，重复刷新不会重复产生历史事件。
- 时间轴同时展示官方 X 公告和本地账户配额观测；公告可在应用内打开原文。
- X 源或 Status 单独不可用时保留另一来源；两者都不可用时显示来源错误并继续保留缓存快照。

当前仍待实现的重点是：后台定时采样账户用量、通知与静默时间、完整社区/GitHub 多源加权预测、历史事件编辑/手动校正，以及 AI 解释层。X 聚合源属于第三方公开转发链路，存在延迟、字段变更和不可达风险，因此它是高置信证据源，不是 OpenAI 官方 API 或重置结果保证。
